...
var calc_event, 
	...
	operation_check = 0, equals_check = 0,
	result_operation = 0, 
	save_status, 
	operation_equals = 0, // переменная отвечает за конструкцию х*=
	check_dot = 0;
...
// 	}
	...
	// 	check_dot = 1;
	// }
	// console.log("check_dot=", check_dot);
...
function func_basic_operation(calc_event) {
	...
	// замена действия при повторном нажатии + - * /
	if (operation_check == 1) {
...
function func_sqrt () {
	...
	if (per_sqrt >= 0) {
		console.log("корень из ", per_sqrt, " = ", Math.sqrt(per_sqrt));
		if (operation_check == 0) {operand_1 = Math.sqrt(per_sqrt); calc_display.value = operand_1;};
		if (operation_check == 2) {operand_2 = Math.sqrt(per_sqrt); calc_display.value = operand_2;};
		calc_status.value += "sqrt(" + per_sqrt + ")";
	} else {
		func_error();
		calc_display.value = "Корень из отриц. числа недопустим";
	}
}
